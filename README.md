...
#### ✏️ 공공의 Open API에서 Body에 `status code`와 `message`를 함께 넣어주는 경우가 있는데 이처럼 Body에 넣어줘야할지 고민함 

```
Body에 status code와 message를 넣어주는 패턴은 안티패턴이라 요즘 잘 안쓰인다고 함

Body에 넣어주면 status code와 message를 알기위해서 일일히 파싱해야하고

오류가 발생하면 파싱할 수 없으므로 status code를 알 수 없는 문제가 있다고 함 
```

#### ✏️ task의 Id를 서버에서 생성해서 클라이언트에게 줘야하는지 아니면 클라이언트에서 생성해서 서버에 보내줘야하는지 고민함

```
서버에서 Id를 생성해서 클라이언트에게 보내주면 해당 앱은 서버에 의존적인 앱이 되므로 

클라이언트에서 Id를 만들어서 서버에 보내준 후 만약 동일한 Id 값이 서버에 존재한다면 

서버에서 Id를 생성해서 클라이언트에게 보내주면 클라이언트는 기존의 Id를 서버에서 새로 받은 Id로 변경하는 방식으로 생각함
```
